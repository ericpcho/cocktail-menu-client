...
export class CocktailCard extends React.Component {
  onClick(event) {
    ...
    const filterByCocktailName = this.props.cocktails.filter(function (cocktail) {
    ...
    if (this.props.view === 'chooseLiquor') {
      ...
      const filterByBaseLiquid = this.props.cocktails.filter(function (cocktail) {
        return cocktail.baseLiquid.includes(selectedItem);
      })
      this.props.dispatch(actions.filterCocktailsByBase(filterByBaseLiquid, selectedItem));
    }
  ...
  render() {
    if (this.props.view === 'chooseDrink' || this.props.view === 'recipe' || this.props.view === 'search') {
      return (
        <div onClick={e => this.onClick(e)} className='cocktailCard'>
          <p><i className="fa fa-glass" aria-hidden="true"></i>  {this.props.selectedItem}</p>
        </div>
      )
    }
    else {
      return (
        <div onClick={e => this.onClick(e)} className='cocktailCard'>
          <p><i className="fa fa-tint" aria-hidden="true"></i>  {this.props.selectedItem}</p>
        </div>
      )
    }
  }
