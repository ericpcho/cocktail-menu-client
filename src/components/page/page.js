    ...
    if (this.props.view === "home") {
      homeDescription = (
        <div className="appDescription">
          ...
          <p className="intro">
            Welcome to Cocktail Search! Do you ever have a bunch of ingredients
            at home for cocktails but aren't sure what you can make? Have you
            ever wanted to recreate a drink you tried at a bar but don't know
            what the ingredients are? With Cocktail Search, both of these
            problems can be solved. By clicking the buttons below, you can
            either search for drinks by name or select the ingredients you
            currently have to find different cocktails. Each cocktail has its
            own recipe card with an ingredients list and instructions.
          </p>
        </div>
      );
      homeOptions = [
        <HomeOption text="Search Drinks by Name" newView="search" key={0} />,
        <HomeOption
          text="Find Drinks by Ingredients"
          newView="chooseLiquor"
          key={1}
        />
      ];
      ...
      if (this.props.hasCocktail) {
        ...
        cocktailRecipe = (
          <CocktailRecipe
            image={this.props.selectedCocktail[0].thumbnail}
            name={this.props.selectedCocktail[0].cocktailName}
            ingredients={this.props.selectedCocktail[0].ingredients}
            recipe={this.props.selectedCocktail[0].recipe}
          />
        );
      } else {
        search = <Search />;
      ...
      let filteredCocktails = this.props.cocktails.filter(cocktail => {
        return (
          cocktail.cocktailName !== this.props.selectedCocktail[0].cocktailName
        );
      });
